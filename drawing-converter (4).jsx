...
function DrawingConverter() {
  ...
  const [originalImage, setOriginalImage] = React.useState(null); // Store original for redrawing
  ...
  const drawOverlay = (dims) => {
    ...
    img.onload = () => {
      console.log('Drawing overlay, image dimensions:', img.width, 'x', img.height);
      
      canvas.width = img.width;
      canvas.height = img.height;
      
      // Clear and draw fresh image
    ...
    // Use the original image, not the processed one
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file.type === 'application/pdf') {
      ...
      try {
        ...
        setOriginalImage(imageDataUrl); // Save original for redrawing
        setProcessedImage(null);
        setDimensions([]);
        setProcessingStage('');
      } catch (error) {
        console.error('Error converting PDF:', error);
        alert('Error loading PDF. Please try an image file instead.');
      }
      return;
    }
    ...
    if (!file.type.match(/^image\//)) {
      console.error('Not an image or PDF file');
    ...
    reader.onload = (event) => {
      ...
      setOriginalImage(event.target.result); // Save original for redrawing
      setProcessedImage(null);
      setDimensions([]);
      setProcessingStage('');
    };
